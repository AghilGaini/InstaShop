...
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
    ...
    <script type="text/javascript">
        ...
        function CreateCategories(Result) {
            var RowNumbers = Math.ceil(Result.payload.Shops.length / 3);
            ...
            for (i = 0; i < Result.payload.Shops.length; i++) {

                var RowNumber = Math.floor(i / 3);
                var RowCategory = document.getElementById('rowCategory' + RowNumber);

                var CategoryLink = CreateElements('a', [{ 'href': '../Pages/shops.aspx?CategoryID=' + Result.payload.Shops[i].ID }], null, null, RowCategory);

                var Col = CreateElements('div', [{ 'class': 'col-md-4 col-sm-6' }], null, null, CategoryLink);

                var Card = CreateElements('div', [{ 'class': 'card CardStyle' }, { 'style': 'background-color:' + Result.payload.Shops[i].Color }], null, null, Col);

                var CardBody = CreateElements('div', [{ 'class': 'card-body' }], null, null, Card);

                CreateElements('h5', [{ 'class': 'card-title' }], null, Result.payload.Shops[i].Title, CardBody);

                CreateElements('img', [{ 'class': 'card-image card-img' }, { 'src': Result.payload.Shops[i].PicURL1 }], null, Result.payload.Shops[i].Title, CardBody);
            }
        }
